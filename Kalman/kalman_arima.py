...
# --- New functions for CRPS and variogram ---
...
def main():
    dt = 1/24
    train, test = get_data(23,13500)

    # ===== fit & get residuals =====
    x_filt, theta, resid, mu_p, S_p = optimal_kalman(dt, np.array([0.5,0.3,1,0.12]), train)

    # ===== diagnostic plots: histogram, QQ-plot, ACF =====
    ...
    axes[1].get_lines()[1].set_color('red')  # make the fit line red (optional)

    # 3) ACF plot
    sm.graphics.tsa.plot_acf(resid, lags=30, ax=axes[2])
    ...
    l_r, g_r = empirical_variogram(resid,48)
    plt.figure()
    plt.plot(l_r, g_r, 'o-')
    plt.title('Residual Variogram (In‐Sample)')
    plt.xlabel('Lag')
    ...
    crps_oos = [crps_gaussian(mu_f[len(train)+i],
                              S_f[len(train)+i],
